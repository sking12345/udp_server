...
class UdpBase {
  public:
	...
	std::map < uint64, std::map<uint16, struct udp_pack*> > send_map_list;
	/*用于释放发送缓存数据*/
  ...
  public:
	...
	virtual void readable_data(struct udp_pack, struct sockaddr_in addr) {};
	//virtual void recved_data(uint8*, uint32 data_size, uint16 task, uint32 userid, uint64 unique = 0x00) {};	//接受到完整数据回调
	...
	void free_send_data(uint64 unique);
